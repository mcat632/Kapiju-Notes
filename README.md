...
- Aplicație desktop GUI nativă: `ioc-cms gui` sau `ioc-cms desktop`, cu dashboard, taburi, agency setup, case creation, evidence, OSINT/darkweb, media, sandbox, devices, reports și admin theme.
...
export PATH="$HOME/.local/bin:$PATH"  # dacă ~/.local/bin nu este deja în PATH
ioc-cms --help
```

`python3 -m ioc_cms ...` funcționează doar când ești în rădăcina repo-ului sau când `PYTHONPATH` indică repo-ul. După installer, folosește comanda `ioc-cms` din orice director.
...
Aplicația desktop include Dashboard, Agency setup, Cases, Evidence, OSINT & Darkweb, Video/Media, Malware/Ransomware, Automotive, Mobile, Drone, SIM/eSIM, Debit/Credit Cards, Crypto, Reports și Admin Theme. Există și un preview web opțional pentru dezvoltare: `ioc-cms web --host 127.0.0.1 --port 8765`.

## Crearea unui caz și raport automat

```bash
ioc-cms case-init DARKNET-001
ioc-cms report DARKNET-001 --summary "Initial darknet OSINT and IOC review."
```
...
- **Mobile/computer/drone/automotive:** spații de lucru separate pentru artefacte, SIM/eSIM, drone, CAR-CAN, OBD-II, ECU și reparații/investigații tehnice.
- **Financial:** crypto, debit/credit card evidence, tranzacții și referințe exchange/KYC.
- **GUI modern desktop:** aplicație nativă desktop cu dashboard-uri, carduri de caz, timeline, panouri OSINT și export rapid de rapoarte.
